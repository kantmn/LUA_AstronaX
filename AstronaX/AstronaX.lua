    ...
    options_bar = {
      ...
      args = {
        ...
        --bar_arena = {
          ...
          --name = l["bar_arena"],
          --desc = l["bar_help"],
          --get = function() if AstronaXDB[player]["bar_arena"] == 1 then return true else return false end end,
          --set = function(_,v) local new = 0 if v then new = 1 end AstronaXDB[player]["bar_arena"] = new; AstronaX:OnTextUpdate(); end,
        --},
        bar_ho = {
          order = 70, type = "toggle", width = "full",
          name = l["bar_ho"],
...
function AstronaX:GUILDBANKFRAME_OPENED()
  if AstronaXDB[player]["abm"] == 1 or AstronaXDB[player]["abml"] == 1 then
    self:UpdateMoney()
  end
end
...
function AstronaX:CheckIfSpellIsPresent(spell,target)
  ...
  if UnitLevel("player") == maxLevel and UnitName(target) and UnitExists(target) and UnitIsDeadOrGhost(target) == nil and UnitUsingVehicle("player") ~= 1 then
    for i=1,40 do
      if UnitBuff(target,i) ~= nil and string.match(UnitBuff(target,i), spell) then
        last_sound_played[spell] = GetTime()
        spell_found = true
        return
      end
...
function AstronaX:GetUnitAuraUpdates()
    ...
    if self:CheckIfSpellIsPresent("Schattengestalt", "player") == true then
...
function AstronaX:OnTextUpdate()
	...
	--local count_arena = count_arena or GetItemCount(selectionIds[7])
  ...
  AstronaXDB[player]["ilvl_minimum"] = ilvl_minimum
	...
	if UnitLevel("player") == maxLevel and AstronaXDB[player] then
      ...
      AstronaXDB[player]["gearscore"] = 0
		...
		--AstronaXDB[player][selectionIds[7]] = count_arena;
  ...
  --if AstronaXDB[player]["bar_arena"] == 1 then
    --text = text.."|TInterface\\Icons\\spell_holy_championsbond:12|t |cffff0000"..count_arena.."|r"..spacertab
  --end
...
function AstronaX:OnTooltipUpdate()
  if not(isInCombat(player)) then
    ...
    if AstronaXDB[player]["tooltip_chardetails"] == 1 then
      ...
      --self:create_tooltip_col(headers, AstronaXDB[player]["tooltip_arena"], "|TInterface\\Icons\\spell_holy_championsbond:16|t")
      ...
      for i, db_player in pairs(sorted_table) do
        ...
        if ( 
          ...
          --local color_arena = "|cffFF0000"			
          --if v[selectionIds[7]] ~= nil and v[selectionIds[7]] < 1000 then color_arena = "|cff660000" end
          --if v[selectionIds[7]] ~= nil and v[selectionIds[7]] <= 500 then color_arena = color_blacked end
          --v[selectionIds[7]] =  str_pad(string.format('%.1f', math.floor( (v[selectionIds[7]] /1000 * 10^1) + 0.5) / (10^1)),2).." k"
          ...
          --self:create_tooltip_col(cols, AstronaXDB[player]["tooltip_arena"], color_arena..v[selectionIds[7]])
